...
public class QueryTests : WarehouseServiceTestBase
{
    ...
    [Fact]
    public async Task GetPaged_QueriesHasProductsForEachItem()
    {
        ...
        WarehouseRepo.HasProductsAsync(wh1.Id, Arg.Any<CancellationToken>()).Returns(true);
        WarehouseRepo.HasProductsAsync(wh2.Id, Arg.Any<CancellationToken>()).Returns(false);
        var service = CreateService();
